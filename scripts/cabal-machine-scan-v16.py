...
def fast15(asset,venue,px,intra,intra_rs,row):
    ...
    base=b[-17:-1]; bh=max(x['h'] for x in base); bl=min(x['l'] for x in base); br=pct(bh,bl); support=min(x['l'] for x in b[-8:]); inv=support*.996
    interest=(rv is not None and rv>=1.15) or f(row.get('rvol1h'))>=1.15 or f(row.get('turnoverToMarketCap'))>=.04 or intra_rs>=.2
    watch=bool(br<=10 and px>=bh*.97 and intra>=.35 and intra<5 and intra_rs>=.1 and f(row.get('priceChange24hPct'))<25 and interest)
    trigger=bool(watch and px>=bh*.995 and intra>=.60 and intra_rs>=.20 and ((rv is not None and rv>=1.35) or f(row.get('rvol1h'))>=1.35))
    return {'source':src,'rvol15m':round(rv,4) if rv is not None else None,'baseHigh4h15m':bh,'baseLow4h15m':bl,'baseRange4h15mPct':round(br,4),'support15m':support,'invalidation15m':inv,'watch':watch,'trigger':trigger}
...
def result(asset,m,b,venue,btc,pre_lane=False):
    ...
    sec=bool(r.get('secondLegTrigger') and r.get('effortVsResult')!='POOR' and irs>=0); stop=f(ft.get('invalidation15m')) if trg and ft else f(r.get('invalidation')); sd=pct(px,stop) if px>stop>0 else 999
    loc=bool(px>stop>0 and sd<=5 and (not no or px<no) and (head is None or head>=.5) and intra<5 and f(r.get('priceChange24hPct'))<25); pilot=bool(loc and (trg or pre or old or sec))
