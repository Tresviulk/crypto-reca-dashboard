#!/usr/bin/env python3
"""CABAL v1.6.5 PUMP RADAR + PILOT ENTRY. Manual SPOT only; never auto-trades."""
...
d['pumpRadar']=pump[:40]; d['pilotEntries']=pilots[:20]; d['pumpRadarStats']={'fast15Attempts':S['fast15Attempts'],'fast15Success':S['fast15Success'],'fast15SuccessRatio':round(S['fast15Success']/S['fast15Attempts'],4) if S['fast15Attempts'] else 1.0,'fastPumpWatchCount':sum(bool(x.get('fastPumpWatch')) for x in pump),'fastPumpTriggerCount':sum(bool(x.get('fastPumpTrigger')) for x in pump),'pilotEntryCount':len(pilots),'exchangeSyntheticAdded':S['synthetic'],'tailMomentumReserveCount':S['tailMomentum'],'coinGeckoPages':S['cgPages'],'preAccumLimit':mod.PREACCUM_LIMIT,'preAccumSub2mReserve':mod.PREACCUM_SUB2M_RESERVE}; d['executionEngine']={'version':'CABAL_PUMP_PILOT_V1.6.5','manualOnly':True,'autoTrade':False,'pilotMaxPctOfPlannedPosition':25,'whalesRole':'CONFIRMATION_PRIORITY_NOT_MANDATORY_VETO','protectiveStopRequired':True,'fastLayer':'LIVE_INTRAHOUR_PLUS_SELECTIVE_COMPLETED_15M','fresh15mRequiredForBuy':True,'wideBaseEarlyAcceleration':True,'deepTailMomentumReserve':True,'coreAlwaysScan':['AVAX','ETH','SOL']}; d['method']='v1.6.5: v1.6.4 plus mandatory CORE lane for AVAX/ETH/SOL. CORE assets bypass the generic 3B market-cap ceiling and cannot be crowded out by Stage-A retention. Missing CORE data makes main coverage fail instead of disappearing silently. Deep-tail momentum reserve remains active for low-rank exchange-native assets. All BUY NOW 15m/RS/no-chase/stop quality gates remain unchanged; manual SPOT only.'
