...
function riskObj(p,m,prev,current,wasInvalidated=false){
  ...
  let observed='INTACT';if(m){const two=m.m15.slice(-2).every(x=>x.c<inv),one=m.h1.at(-1).c<inv,low=Math.min(...m.m15.slice(-4).map(x=>x.l));observed=one||two?'INVALIDATED':low<inv&&m.m15.at(-1).c>=inv?'RECLAIMED':low<inv?'BREACH':'INTACT'}
  const st=wasInvalidated?'INVALIDATED':observed;
...
async function main(){
  ...
  for(const p of open){let m=markets[p.asset]||null;if(!m&&p.asset==='TRUST'){try{m=await market('TRUST')}catch{}}let current=m?.current??null;if(current==null){try{current=await cgPrice({TRUST:'intuition',PHA:'pha'}[p.asset])}catch{}}const prev=oldRisk?.positionRisk?.positions?.[p.id]||null;const wasInvalidated=prev?.structuralState==='INVALIDATED'||prev?.managementThesisState==='INVALIDATED'||rh.some(r=>r.positionId===p.id&&r.structuralState==='INVALIDATED');const o=riskObj(p,m,prev,current,wasInvalidated);rp[p.id]=o;if(o.prs==null)partial=true;if(prev?.action!==o.action||o.fastDropRisk?.triggered)rh.push({timestampEuropeMadrid:human,positionId:p.id,asset:p.asset,prs:o.prs,action:o.action,structuralState:o.structuralState,protection:o.protection,event:o.fastDropRisk?.triggered?'FAST_DROP_RISK':'AUTONOMOUS_RISK_STATE_CHANGE'})}
