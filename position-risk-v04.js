// Crypto Reca v0.4.8 — Position Risk / Exit Advisory UI
(function(){
  ...
  function riskCard(p,compact=false){
    ...
    if(compact)return `<div class="card risk-engine-card ${r.action==='EXIT SIGNAL'?'risk-critical':''}">${head}<div class="risk-score-row"><div><small>PRS</small><strong>${prs==null?'—':prs}<span>/100</span></strong></div><div><small>Tesis</small><strong>${esc(r.thesisState||'—')}</strong></div><div><small>Riesgo si llega a invalidación</small><strong>${r.modeledLossUSDC==null?'—':`${money(r.modeledLossUSDC)} USDC`}</strong></div></div>${r.trendConclusion?`<div class="tiny">${esc(r.trendConclusion)}</div>`:''}<p>${esc(riskActionText(r.action))}</p>${r.reason?`<div class="tiny">${esc(r.reason)}</div>`:''}</div>`;
